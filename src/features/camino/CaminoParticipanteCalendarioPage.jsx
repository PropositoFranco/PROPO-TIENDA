...
@media (max-width:760px){
  .crk-orb{filter:blur(30px); animation:none; mix-blend-mode:normal; opacity:0.18;}
}
...
export default function CaminoParticipanteRankingPage() {
  ...
  async function cargar() {
    ...
    const nuevo = data || [];
    setRanking(prev => {
      // Evita re-render (y el jank de scroll que causaba) si los datos no cambiaron realmente
      if (prev.length === nuevo.length && JSON.stringify(prev) === JSON.stringify(nuevo)) {
        return prev;
      }
      return nuevo;
    });
    ...
    if (!ligaAutoSeleccionada.current) {
      const yo = nuevo.find((p) => p.participante_id === sessionData.session.user.id);
